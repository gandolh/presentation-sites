#!/usr/bin/env node
/**
 * AXA Tractări deploy tool — zero dependencies, native Node TypeScript.
 *
 ...
 *
 * NOTE: AXA Tractări is a DEMO/concept site. The build is fully self-contained
 * (the hero is a Three.js scene, no external assets to swap); deploy just builds
 * with PUBLIC_BASE and uploads dist/.
 */
...
function syncCaddyfile(cfg: Config) {
  ...
  const tmp = "/tmp/Caddyfile.tractari";
  run("scp", [
    ...sshArgsForScp(cfg),
    CADDYFILE,
    `${cfg.sshTarget}:${tmp}`,
...
function deploy(cfg: Config, opts: { build: boolean }) {
  ...
  if (opts.build) {
    ...
    // Demo site → ships the committed SVG mockups (mock is the default source).
    run("npm", ["run", "build"], {
      cwd: PROJECT_ROOT,
      // PUBLIC_BASE is injected into the env by the caller (withBaseEnv).
    });
  } else {
...
function printHelp() {
  console.log(`
${c.bold}AXA Tractări deploy${c.reset} — zero-dependency Node TypeScript deploy tool.
