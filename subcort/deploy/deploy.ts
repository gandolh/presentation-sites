#!/usr/bin/env node
/**
 * Subcort deploy tool — zero dependencies, native Node TypeScript.
 *
 ...
 *
 * NOTE: Subcort is a DEMO/concept site — it ships the committed SVG mockups
 * (PUBLIC_IMAGE_SOURCE defaults to "mock"); there are no real photos to swap in.
 */
...
function syncCaddyfile(cfg: Config) {
  ...
  const tmp = "/tmp/Caddyfile.subcort";
  run("scp", [
    ...sshArgsForScp(cfg),
    CADDYFILE,
    `${cfg.sshTarget}:${tmp}`,
...
function printHelp() {
  console.log(`
${c.bold}Subcort deploy${c.reset} — zero-dependency Node TypeScript deploy tool.
